         ...
         IF (J.EQ.1) THEN
            ...
            IF (ICKWRK(IcNS+I-1) .LT. 0) THEN
               IF (LT+2 .GT. ILEN) THEN
                  ...
                  RETURN
               ENDIF
C     Luwi Oluwole--10/21/2003--changed format for xml output
c               ISTR(LT+1:) = '=>'
               ISTR(LT+1:) = '=&gt;'
C     End Luwi O.
            ELSE
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKWCSPARSE   (T, C, CTOT, ICKWRK, RCKWRK, WDOT)
C
C  START PROLOGUE
C
C DAS 000414 --- Sparse implementation of CKWC.  By passing through
C                CTOT, we will not get dense rows due to third body
C                concentration calculations.
C
C  SUBROUTINE CKWCSPARSE   (T, C, CTOT, ICKWRK, RCKWRK, WDOT)
C     Returns the molar production rates of the species given the
C     temperature and molar concentrations;  see Eq. (49).
C
C  INPUT
...
C                   Dimension C(*) at least KK, the total number of
C                   species.
C     CTOT   - Total molar concentration of mixture.
C                   cgs units - mole/cm**3
C                   Data type - real array
C     ICKWRK - Array of integer workspace.
C                   Data type - integer array
...
C
   ...
   25 CONTINUE
C
C DAS 000414 --- Calls the sparse CKRATX implementation
      CALL CKRATXSPARSE (NII, NKK, MXSP, MXTB, T, RCKWRK(NcK1), CTOT,
     1             ICKWRK(IcNS), ICKWRK(IcNU), ICKWRK(IcNK), NPAR+1,
     2             RCKWRK(NcCO), NFAL, ICKWRK(IcFL), ICKWRK(IcFO),
     3             ICKWRK(IcKF), NFAR, RCKWRK(NcFL), NTHB,ICKWRK(IcTB),
     4             ICKWRK(IcKN),RCKWRK(NcKT),ICKWRK(IcKT),RCKWRK(NcKF),
     5             RCKWRK(NcKR),RCKWRK(NcI1), RCKWRK(NcI2),
     6             RCKWRK(NcI3),NRNU, ICKWRK(IcRNU), RCKWRK(NcRNU),
     7             NORD, ICKWRK(IcORD), MXORD, ICKWRK(IcKOR),
     8             RCKWRK(NcKOR))
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKRATXSPARSE
     1               (II, KK, MAXSP, MAXTB, T,C,CTOT, NSPEC, NU, NUNK,
     2                NPAR, PAR, NFAL, IFAL, IFOP, KFAL, NFAR, FPAR,
     3                NTHB, ITHB, NTBS, AIK, NKTB, RKFT, RKRT, RKF,
     4                RKR, CTB, NRNU, IRNU, RNU, NORD, IORD, MXORD,
     5                KORD, RORD)
C
C  START PROLOGUE
C
C DAS 000414 --- This is a hopefully a sparser implementation of
C                CKRATX.  The only change is that the
C                total concentration is passed to the subroutine via
C                CTOT.
C  SUBROUTINE CKRATXSPARSE
C 1                  (II, KK, MAXSP, MAXTB, T,C,CTOT, NSPEC, NU, NUNK,
C 2                   NPAR, PAR, NFAL, IFAL, IFOP, KFAL, NFAR, FPAR,
C 3                   NTHB, ITHB, NTBS, AIK, NKTB, RKFT, RKRT, RKF,
C 4                   RKR, CTB, NRNU, IRNU, RNU, NORD, IORD, MXORD,
C 5                   KORD, RORD)
C
C  END PROLOGUE
C
...
C
      IF (NTHB .GT. 0) THEN
C DAS 000414 --- We pass CTOT as a parameter now, hopefully making it sparser
C         CTOT = 0.0
C         DO 10 K = 1, KK
C            CTOT = CTOT + C(K)
C   10    CONTINUE
